...
class Main():
    ...
    def load_qualities(self, row, col):
        self.current_streamer = self.table_streams.item(row, 0).text()
        self.combo_quality.clear()
        index = 0
        for quality in self.twitch.get_qualities(self.current_streamer):
    ...
    def watch_stream(self, boolean):
        url = "https://twitch.tv/%s" % (self.current_streamer)
        print url
        coding = "-p 'C:\Program Files (x86)\VideoLAN\VLC\\vlc.exe' --file-caching 4000 --network-caching 4000"
        quality = str(self.combo_quality.currentText())
        print quality
        Popen(["livestreamer", coding, url, quality,
               "--hls-segment-threads", "3", "--player-continuous-http" ]
              , shell=False
              ,  startupinfo=si
